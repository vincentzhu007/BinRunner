...
static unsigned long loadelf_anon(int fd, Elf_Ehdr *ehdr, Elf_Phdr *phdr)
{
	...
	for (iter = phdr; iter < &phdr[ehdr->e_phnum]; iter++) {
		...
		if (z_read(fd, p + off, iter->p_filesz) !=
			...
			goto err;
		{
			/* HarmonyOS: debug 应用需要用 jit prctl 放开匿名页可执行权限
			 * （0x6a6974 == "jit"），否则 mprotect PROT_EXEC 被 SELinux 拒绝 */
...
void z_entry(unsigned long *sp, void (*fini)(void))
{
	...
	for (i = 0;; i++, ehdr++) {
		 ...
		 * HarmonyOS 定制：首轮文件名形如 "@<fd>" 时直接使用已打开的 fd
		 * （App 沙箱内 /proc/self/fd 不可访问，无法通过路径重开 memfd） */
		if (i == 0 && file[0] == '@') {
			fd = 0;
			for (const char *s = file + 1; *s >= '0' && *s <= '9'; s++)
				fd = fd * 10 + (*s - '0');
		} else if ((fd = z_open(file, O_RDONLY)) < 0)
